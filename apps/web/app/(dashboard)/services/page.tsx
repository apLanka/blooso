...
export default function ServicesPage() {
  ...
  const handleDeleteCategory = async (categoryId: string) => {
    if (
      !token ||
      !currentBusinessId ||
      !confirm('Are you sure you want to delete this category and all its services?')
    )
      return;
    try {
  ...
  const handleDeleteService = async (serviceId: string) => {
    if (!token || !currentBusinessId || !confirm('Are you sure you want to delete this service?'))
      return;
    try {
    ...
    return (
      <div className="flex h-[60vh] flex-col items-center justify-center text-center">
        ...
        <p className="mb-6 text-muted-foreground">
          Apply to list your business on Blooso to manage services.
        </p>
        <button
      ...
      {totalServices === 0 && categories.length === 0 ? (
        ...
        <div className="space-y-8">
          {categories.map((cat) => (
            <div
              key={cat.id}
              className="rounded-[24px] bg-white p-6 shadow-sm lg:p-8 overflow-hidden border"
              style={{ borderColor: 'var(--blooso-border-light)' }}
            >
              <div
            ...
            >
              ...
              <div>
                <label className="mb-2 block text-sm font-bold text-black">
                  Description <span className="text-black/40 font-medium">(Optional)</span>
                </label>
                <textarea
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setServiceForm((s) => ({ ...s, price: parseFloat(e.target.value) || 0 }))
                    }
                    className="w-full rounded-[12px] border px-4 py-3 text-sm outline-none transition-all"
                    style={{
                      borderColor: 'var(--blooso-border)',
              ...
              <div
                className="flex items-center justify-between rounded-[16px] bg-[#F9F7F5] p-4 border"
                style={{ borderColor: 'var(--blooso-border-light)' }}
              >
                <div>
                  <p className="font-bold text-black">Active Service</p>
                  <p className="text-xs font-medium text-black/60 mt-0.5">
                    Allow clients to book this service
                  </p>
                </div>
                <button
                  type="button"
                ...
                >
                  <span
                    className={cn(
                      'inline-block h-4 w-4 transform rounded-full bg-white transition-transform',
                      serviceForm.isActive ? 'translate-x-6' : 'translate-x-1'
                    )}
                  />
                </button>
              </div>
